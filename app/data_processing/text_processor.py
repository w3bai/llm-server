...
class TextProcessor:
    ...
    def split_large_chunk(self, chunk: str, max_tokens: int = 7500) -> List[str]:
        ...
        for sentence in sentences:
            if (
                self.estimate_tokens(" ".join(current_sub_chunk) + " " + sentence)
                > max_tokens
            ):
        ...
        for sub_chunk in sub_chunks:
            if self.estimate_tokens(sub_chunk) > max_tokens:
                ...
                for word in words:
                    if (
                        self.estimate_tokens(" ".join(current_word_chunk) + " " + word)
                        > max_tokens
                    ):
    ...
    def get_remaining_code(self, tree: ast.AST, code: str) -> str:
        ...
        remaining_lines = [
            line for i, line in enumerate(code_lines) if i not in used_lines
        ]
    ...
    def generate_embedding(self, text: str):
        if self.estimate_tokens(text) > 8000:
            raise ValueError(
                f"Text is too long ({self.estimate_tokens(text)} tokens). Maximum is 8000 tokens."
            )
